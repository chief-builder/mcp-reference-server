...
export class TokenManager {
     ...
     */
    async getValidAccessToken(resource) {
        const key = this.getResourceKey(resource);
     ...
     */
    isPayloadExpired(payload, toleranceSeconds = 0) {
     ...
     */
    validateJwtFormat(token, options) {
    ...
    async introspect(token, tokenTypeHint) {
        ...
        if (!response.ok) {
            throw new TokenError('introspection_failed', `Token introspection failed with status ${response.status}`);
        }
     ...
     */
    async validateWithIntrospection(token) {
        try {
     ...
     */
    async refreshTokenIfPossible(key, token) {
     ...
     */
    async performRefresh(key, token) {
 ...
 * @throws {TokenValidationError} If validation fails
 */
export async function validateAccessToken(token, options) {
 ...
 */
export async function refreshAccessToken(refreshToken, tokenEndpoint, clientId, clientSecret) {
    const body = new URLSearchParams();
