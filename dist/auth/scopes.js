 ...
 */
import { z } from 'zod';
import { buildWwwAuthenticateHeader } from './discovery.js';
// =============================================================================
// Constants
// =============================================================================
...
// Zod Schemas
// =============================================================================
export const ScopeStringSchema = z.string().refine((val) => val.trim().length > 0, { message: 'Scope string cannot be empty' });
 ...
 */
export function getToolNameFromScope(scope) {
 ...
 */
export function buildToolScope(toolName) {
...
export class ScopeManager {
     ...
     */
    hasScopeWithInheritance(tokenScopes, required) {
        const scopes = Array.isArray(tokenScopes) ? tokenScopes : parseScopes(tokenScopes);
        // Direct match
     ...
     */
    checkScopes(tokenScopes, required) {
        const scopes = Array.isArray(tokenScopes) ? tokenScopes : parseScopes(tokenScopes);
        const missingScopes = [];
     ...
     */
    getRequiredScopes(method, toolName) {
     ...
     */
    validateMethodAccess(tokenScopes, method, toolName) {
        const scopes = Array.isArray(tokenScopes) ? tokenScopes : parseScopes(tokenScopes);
        ...
        if (!result.allowed && result.missingScopes) {
            throw new InsufficientScopeError(result.missingScopes, scopes, `Access denied for ${method}: ${result.message}`);
        }
    }
    /**
     ...
     * @throws {InsufficientScopeError} If scopes are insufficient
     */
    validateToolAccess(tokenScopes, toolName, requireToolScope = true) {
        const scopes = Array.isArray(tokenScopes) ? tokenScopes : parseScopes(tokenScopes);
        // Check base method scope (mcp:write)
        const result = this.checkScopes(scopes, [MCP_SCOPES.WRITE]);
        if (!result.allowed && result.missingScopes) {
            throw new InsufficientScopeError(result.missingScopes, scopes, `Access denied for tool ${toolName}: requires ${MCP_SCOPES.WRITE} scope`);
        }
        // Check tool-specific scope if required
        if (requireToolScope) {
            const toolScope = buildToolScope(toolName);
            if (!scopes.includes(toolScope)) {
                throw new InsufficientScopeError([toolScope], scopes, `Access denied for tool ${toolName}: requires ${toolScope} scope`);
            }
        }
    }
    /**
    ...
    build403Response(error) {
        ...
        return {
            status: 403,
            headers: {
                'WWW-Authenticate': error.buildWwwAuthenticateHeader(resourceMetadataUrl, this.config.realm),
 ...
 */
export function checkScopeWithInheritance(tokenScopes, required) {
 ...
 */
export function getAllMcpScopes() {
    return [MCP_SCOPES.READ, MCP_SCOPES.WRITE, MCP_SCOPES.ADMIN];
}
/**
 * Get all tool scopes.
 */
export function getAllToolScopes() {
