...
export const PKCE_VERIFIER_CHARSET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~';
/**
 ...
 */
export function generateCodeVerifier(length = PKCE_VERIFIER_DEFAULT_LENGTH) {
    if (length < PKCE_VERIFIER_MIN_LENGTH || length > PKCE_VERIFIER_MAX_LENGTH) {
        throw new Error(`Code verifier length must be between ${PKCE_VERIFIER_MIN_LENGTH} and ${PKCE_VERIFIER_MAX_LENGTH}, got ${length}`);
    }
 ...
 */
export async function generateCodeChallengeAsync(verifier) {
 ...
 */
export function generatePKCEChallenge(length) {
