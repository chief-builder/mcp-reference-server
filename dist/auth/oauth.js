 ...
 */
export function getAuthorizationEndpoint(issuer) {
 ...
 */
export function getTokenEndpoint(issuer) {
    const baseUrl = issuer.endsWith('/') ? issuer.slice(0, -1) : issuer;
...
export class OAuthClient {
     ...
     */
    async handleCallback(params, session) {
        // Check for authorization error
        if (params.error) {
            throw new OAuthError(params.error, params.errorDescription ?? params.error);
        }
    ...
    async requestToken(body) {
        ...
        if (!response.ok) {
            ...
            throw new OAuthError('server_error', `Token request failed with status ${response.status}`);
        }
 ...
 */
export function normalizeTokenResponse(response) {
    const result = {
 ...
 */
export function createAuth0Client(domain, clientId, options = {}) {
