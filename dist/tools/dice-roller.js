...
// =============================================================================
export const diceRollerInputJsonSchema = {
    type: 'object',
    properties: {
        notation: {
            type: 'string',
            description: 'Dice notation in NdS[+/-M] format. N = number of dice (optional, default 1), d = literal "d", S = sides (4, 6, 8, 10, 12, 20, 100), +/-M = optional modifier. Examples: d20, 2d6, 1d20+5, 3d8-2',
        },
    },
    required: ['notation'],
 ...
 */
export async function rollDiceHandler(args) {
    // Validate input with Zod
    const parseResult = DiceRollerInputSchema.safeParse(args);
    if (!parseResult.success) {
        return createToolErrorResult('Invalid input: ' + parseResult.error.errors.map((e) => e.message).join(', '), 'roll_dice');
    }
    const { notation } = parseResult.data;
    // Parse the dice notation
    const parsed = parseDiceNotation(notation);
    if (!parsed) {
        return createToolErrorResult(`Invalid dice notation: "${notation}". ` +
            `Expected format: NdS[+/-M] where N is number of dice (1-${MAX_DICE}), ` +
            `S is sides (${VALID_SIDES.join(', ')}), and +/-M is optional modifier. ` +
            'Examples: d20, 2d6, 1d20+5, 3d8-2', 'roll_dice');
    }
    // Roll the dice
...
export const diceRollerTool = {
    name: 'roll_dice',
    title: 'Dice Roller',
    description: 'Roll dice using standard tabletop notation. Supports common die types (d4, d6, d8, d10, d12, d20, d100) with optional modifiers. Returns individual roll results and total.',
 ...
 */
export function registerDiceRollerTool(registry) {
