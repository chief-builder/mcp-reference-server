...
// =============================================================================
export const fortuneTellerInputJsonSchema = {
    type: 'object',
    properties: {
        category: {
            type: 'string',
        ...
        mood: {
            type: 'string',
            enum: [...MOODS],
            description: 'The tone of the fortune: optimistic (positive), mysterious (enigmatic), or cautious (warning). Defaults to mysterious.',
        },
    },
    required: ['category'],
 ...
 */
export function selectFortune(category, mood) {
 ...
 */
export function getFortuneCompletions(argName, prefix = '') {
 ...
 */
export async function tellFortuneHandler(args) {
    ...
    if (!parseResult.success) {
        ...
        for (const err of errors) {
            if (err.path[0] === 'category') {
                return createToolErrorResult(`Invalid category. Must be one of: ${CATEGORIES.join(', ')}`, 'tell_fortune');
            }
            if (err.path[0] === 'mood') {
                return createToolErrorResult(`Invalid mood. Must be one of: ${MOODS.join(', ')}`, 'tell_fortune');
            }
        }
        return createToolErrorResult('Invalid input: ' + errors.map((e) => e.message).join(', '), 'tell_fortune');
    }
...
export const fortuneTellerTool = {
    ...
    description: 'Reveal a fortune for the querent. Choose a category (love, career, health, wealth, general) and optionally a mood (optimistic, mysterious, cautious) to set the tone of the reading.',
 ...
 */
export function registerFortuneTellerTool(registry) {
