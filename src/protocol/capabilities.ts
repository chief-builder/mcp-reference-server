...
import { JsonRpcErrorCodes, createJsonRpcError, type JsonRpcError } from './jsonrpc.js';
...
export class CapabilityManager {
  ...
  requireClientCapability(path: CapabilityPath): void {
    if (!this.hasClientCapability(path)) {
      throw new CapabilityError(
        `Client does not support required capability: ${path}`,
        { requiredCapability: path }
      );
    }
  }
  ...
  requireServerCapability(path: CapabilityPath): void {
    if (!this.hasServerCapability(path)) {
      throw new CapabilityError(
        `Server does not support required capability: ${path}`,
        { requiredCapability: path }
      );
    }
  }
  ...
  validateMethodCapability(method: string): void {
    ...
    if (requiredCapability && !this.hasServerCapability(requiredCapability)) {
      throw new CapabilityError(
        `Method '${method}' requires capability '${requiredCapability}' which is not supported`,
  ...
  validateNotificationCapability(notificationMethod: string): void {
    if (!this.canSendNotification(notificationMethod)) {
      ...
      const requiredCapability = notificationMapping[notificationMethod];
      throw new CapabilityError(
        `Cannot send '${notificationMethod}' notification: client does not support '${requiredCapability}'`,
        { notificationMethod, requiredCapability }
      );
    }
  }
}
 ...
 */
export function hasCapabilityAtPath(
 ...
 */
export function getCapabilityAtPath(
  capabilities: ClientCapabilities | ServerCapabilities,
  path: CapabilityPath
): unknown {
  const parts = path.split('.');
 ...
 */
export function getNotificationCapabilityMapping(): Record<string, CapabilityPath> {
  return {
...
export function requireCapability(
  ...
  if (!hasCapabilityAtPath(capabilities, name)) {
    throw new CapabilityError(
      `Required capability not present: ${name}`,
